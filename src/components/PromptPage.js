...
export default function PromptPage() {
    ...
    return (_jsx("div", { className: "min-h-screen bg-white", children: _jsxs("div", { className: "max-w-4xl mx-auto px-6 py-16 sm:px-8 sm:py-20", children: [_jsxs("div", { className: "mb-12", children: [_jsx("h1", { className: "text-5xl sm:text-6xl font-serif font-semibold text-gray-900 mb-4", children: "Reconstruction Prompt" }), _jsx("p", { className: "text-lg text-gray-600 mb-8", children: "Complete blueprint for rebuilding this website. Use this prompt with another LLM to recreate the experience." }), _jsx("button", { onClick: handleCopyPrompt, className: "inline-flex items-center gap-2 px-6 py-3 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-yellow-600", children: copied ? (_jsxs(_Fragment, { children: [_jsx(Check, { size: 18 }), "Copied to clipboard"] })) : (_jsxs(_Fragment, { children: [_jsx(Copy, { size: 18 }), "Copy Prompt"] })) })] }), _jsx("div", { ref: promptRef, className: "prose prose-lg max-w-none text-gray-700", style: {
                        fontFamily: 'Inter, system-ui, sans-serif',
                        lineHeight: '1.8',
                    }, children: RECONSTRUCTION_PROMPT.split('\n').map((line, i) => {
                        if (line.startsWith('# ')) {
                            return (_jsx("h1", { className: "text-4xl font-serif font-semibold text-gray-900 mt-12 mb-6", children: line.replace('# ', '') }, i));
                        }
                        if (line.startsWith('## ')) {
                            return (_jsx("h2", { className: "text-2xl font-serif font-semibold text-gray-900 mt-10 mb-4", children: line.replace('## ', '') }, i));
                        }
                        if (line.startsWith('**') && line.includes(':')) {
                            const [label, ...rest] = line.split(':');
                            return (_jsxs("p", { className: "text-gray-700 mb-2", children: [_jsxs("strong", { children: [label.replace(/\*\*/g, ''), ":"] }), rest.join(':').trim()] }, i));
                        }
                        if (line.startsWith('- ')) {
                            return (_jsxs("p", { className: "text-gray-700 ml-6 mb-1", children: ["\u2022", ' ', _jsx("span", { children: line.substring(2) })] }, i));
                        }
                        if (line.trim() === '') {
                            return _jsx("div", { className: "h-2" }, i);
                        }
                        return (_jsx("p", { className: "text-gray-700 mb-3", children: line }, i));
                    }) }), _jsx("div", { className: "mt-16 pt-12 border-t border-gray-200", children: _jsx(Link, { to: "/", className: "inline-flex items-center gap-2 text-gray-900 hover:text-yellow-600 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-yellow-600 rounded-lg px-4 py-2 -ml-4", children: "<- Back to Experience" }) })] }) }));
}
